    ...
    <div class="col s12 m12 l12" style="min-height: 660px;">
        ...
        @else
            <table class="striped centered responsive-table">
                ...
                <tbody>
                @foreach($book_images as $index => $data)
                    <tr>
                        <td>{{$index+1}}</td>
                        <td>
                            <img class="materialboxed" width="550" src="{{URL::asset('images/banners/'.$data->name)}}">
                        </td>
                        <td>{{$data['big_text']}}</td>
                        <td>{{$data['small_text']}}</td>
                        <td>{{$data->image_type->position}}</td>
                        <td>
                            <a class="btn-floating waves-effect waves-light gradient-45deg-amber-amber gradient-shadow tooltipped"
                               ...
                               href="{{ route('book-image.edit', ['id'=>$data['id'],'book_id'=>$book_id]) }}">
                            ...
                            <a class="btn-floating btn-warning-cancel waves-effect waves-light red tooltipped"
                               ...
                               onclick="deleteModal('{{ route('book-image.destroy', ['id'=>$data['id'],'book_id'=>$book_id]) }}')">
        ...
        <div class="fixed-action-btn " style="bottom: 50px; right: 19px;">
            <a class="btn-floating btn-large waves-effect waves-light gradient-45deg-light-blue-cyan gradient-shadow tooltipped" data-position="top" data-delay="10" data-tooltip="Crear" href="{{route('book-image.create', ['book_id'=>$book_id])}}">
